  ...
  const handleLogin = async (email: string, pass: string) => {
    try {
      const response = await fetch("http://localhost:8080/api/auth/login", {
        ...
        body: JSON.stringify({
          username: email, // assuming email is used as username for admin
          password: pass,
        }),
      });
      if (response.ok) {
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex space-x-4">
          <Button
            ...
            className={cn(
              "rounded-xl h-12 flex-1 md:flex-none md:w-48 font-bold text-md",
              activeModule === "jobhub" ? "bg-primary text-white" : "",
            )}
          ...
          <Button
            ...
            className={cn(
              "rounded-xl h-12 flex-1 md:flex-none md:w-48 font-bold text-md",
              activeModule === "marketplace" ? "bg-primary text-white" : "",
            )}
