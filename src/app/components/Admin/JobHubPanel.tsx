...
export function JobHubPanel() {
    ...
    // TODO: Fetch Job Hub data from API
    useEffect(() => {
        // Example: fetchJobHubProfiles().then(setProfiles);
        // Example: fetchJobReports().then(setReports);
    }, []);
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <Card className={cn("cursor-pointer transition-all hover:border-destructive/50", activeTab === "reported" && "border-destructive ring-1 ring-destructive")} onClick={() => setActiveTab("reported")}>
                    <CardContent className="p-6 flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Reported Jobs</p>
                                            ...
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <h3 className="font-bold text-foreground flex items-center gap-2">
                                                        {profile.legalName}
                                                        <Badge variant="outline" className="text-[10px] uppercase font-bold">{profile.accountType}</Badge>
                                                    </h3>
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                                    <div className="space-y-4">
                                                        ...
                                                        <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-sm">
                                                            ...
                                                            <div className="text-muted-foreground">Contact No:</div><div className="font-medium">{profile.contactNo}</div>
                                                            <div className="text-muted-foreground">Business URL:</div><div className="font-medium text-primary hover:underline">{profile.url}</div>
                                                            <div className="text-muted-foreground">HQ Location:</div><div className="font-medium">{profile.hqLocation}</div>
                                                            {profile.accountType === "corporate" && (
                                                                <><div className="text-muted-foreground">BR Number:</div><div className="font-medium">{profile.brNumber}</div></>
                                                            )}
                                                            {profile.accountType === "individual" && (
                                                                <><div className="text-muted-foreground">NIC/Passport:</div><div className="font-medium">{profile.nicPassport}</div></>
                                                            )}
                                                            <div className="text-muted-foreground col-span-2 mt-2">Overview:</div>
                                                            <div className="font-medium col-span-2 bg-white p-3 rounded-lg border border-border/50">{profile.overview}</div>
                                                        </div>
                                                    </div>
                                                    <div className="space-y-4">
                                                        ...
                                                        <div className="flex flex-col gap-3">
                                                            ...
                                                            {profile.accountType === "corporate" && (
                                                                <Button variant="outline" className="justify-start h-12 rounded-xl bg-white border-primary/20 hover:bg-primary/5"><ImageIcon className="mr-3 text-primary" size={18} /> Review BR Certificate</Button>
                                                            )}
                                                            {profile.accountType === "individual" && (
